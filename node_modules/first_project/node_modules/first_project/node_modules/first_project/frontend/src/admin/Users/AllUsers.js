...
const AllUsers = () => {
  ...
  return (
    <div className="w-full p-6 mx-auto rounded-lg shadow-lg bg-white-200">
      <h1 className="mb-4 text-xl font-semibold">Admin - Users Management</h1>
      ...
      <form onSubmit={isEditing ? handleUpdateUser : handleCreateUser} className="p-4 mb-6 space-y-4 bg-white rounded-lg shadow-md">
        <h2 className="text-sm text-xl font-semibold">{isEditing ? 'Edit User' : 'Create New User'}</h2>
        <input 
          type="text" 
          placeholder="Name" 
          value={newUser.name} 
        ...
        <input 
          ...
          className="w-full p-2 text-sm border rounded"
        />
        <button type="submit" disabled={loading} className="px-4 py-2 text-sm text-white bg-blue-600 rounded">
      ...
      {users.length > 0 ? (
        <table className="w-full overflow-hidden text-sm bg-white rounded-lg shadow-md">
          <thead className="bg-gray-200">
            <tr>
              ...
              <th className="px-4 py-2 text-sm">Admin</th>
          ...
          <tbody>
            {users.map((user) => (
              <tr key={user._id} className="border-b">
                ...
                <td className="px-4 py-2 text-sm">{`${user.street}, ${user.apartment}, ${user.city}, ${user.country}`}</td>
                <td className="px-4 py-2 text-sm">{user.isAdmin ? 'Yes' : 'No'}</td>
                <td className="px-4 py-2 text-sm">
                  <button onClick={() => handleEditUser(user)} className="px-2 py-1 mx-1 text-sm text-white bg-yellow-500 rounded">Edit</button>
                  <button onClick={() => handleDeleteUser(user._id)} className="px-2 py-1 mx-1 text-sm text-white bg-red-500 rounded">Delete</button>
                </td>
              </tr>
            ))}
          </tbody>
        ...
        <p className="mt-4 text-sm text-gray-500">No users found. Please check your API or database.</p>
      )}
    </div>
  );
};
